...
└── RTL_Refinement_Notes.md         # Ghi chú lý thuyết thuật toán và gán chân FPGA
```
...
> [!NOTE]
> Thư mục cấu hình mô phỏng (`sim/` chứa `Makefile`) và file dự án Vivado (`cordic.xpr`) là các thành phần của phần cứng ban đầu, nằm ở thư mục gốc của dự án cục bộ. Nếu bạn muốn chạy các lệnh mô phỏng này hoặc mở project Vivado trực tiếp trong repository này, hãy sao chép thư mục `sim/` và file `cordic.xpr` từ thư mục `CORDIC/` gốc vào thư mục repository `CORDIC_nhom1/`.
...
Truy cập vào thư mục `sim/` (sau khi đã copy vào repo) trong terminal và sử dụng các lệnh `make`:
...
2. Chọn **Open Project** và tìm đến file `cordic.xpr` (sau khi đã copy vào repo).
...
Nếu bạn cần cấu hình chân trên kit FPGA vật lý **Artix-7 (XC7A35T-1CPG236C)**, vui lòng xem chi tiết đặc tả file ràng buộc (`constraints.xdc`) trong mục 3 của file [RTL_Refinement_Notes.md](RTL_Refinement_Notes.md).